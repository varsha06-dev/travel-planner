...
export default function RouteMap({ routeData }) {
  ...
  useEffect(() => {
    ...
    // Use streets-v12 — shows road names, POIs, building labels
    ...
    map.on('load', () => {
      ...
      if (dayData.route_geometry) {
        ...
        map.addLayer({
          ...
          paint: { 'line-color': '#C9A96E', 'line-width': 4 },
        })
        map.addLayer({
          id: 'route-dash', type: 'line', source: 'route',
          paint: { 'line-color': '#fff', 'line-width': 1.5,
                   'line-dasharray': [3, 5], 'line-opacity': 0.6 },
        })
      }
      ...
      dayData.stops.forEach((stop, i) => {
        ...
        const popupHTML = `
          <div style="font-family:'DM Sans',sans-serif;padding:4px 6px;min-width:160px">
            <p style="font-size:0.88rem;font-weight:600;color:#1C1917;margin:0 0 3px">
              ${stop.name}
            </p>
            <p style="font-size:0.73rem;color:#78716C;margin:0;text-transform:capitalize">
              ${STOP_ICONS[stop.type] || '📍'} ${stop.type}
            </p>
            ${stop.duration_to_next_min ? `
              <p style="font-size:0.72rem;color:#C9A96E;margin:6px 0 0;
                        border-top:1px solid #E8E6E1;padding-top:5px">
                → ${stop.duration_to_next_min} min drive${stop.distance_to_next_km > 0 ? ` · ${stop.distance_to_next_km} km` : ''}
              </p>` : ''}
          </div>`
      ...
      // ── Fit to all stops with tight padding ──────────────────────────
      if (dayData.stops.length === 1) {
        map.flyTo({ center: [dayData.stops[0].lng, dayData.stops[0].lat], zoom: 15 })
      } else {
        const bounds = new mapboxgl.LngLatBounds()
        dayData.stops.forEach(s => bounds.extend([s.lng, s.lat]))
        map.fitBounds(bounds, {
          padding:  { top: 60, bottom: 60, left: 60, right: 60 },
          maxZoom:  16,    // street level — shows individual buildings
          duration: 800,
        })
      }
    })
                    ...
                    <div style={styles.connector}>
                      ...
                      {stop.duration_to_next_min && (
                        <div style={styles.connectorLabel}>
                          ...
                          {stop.distance_to_next_km > 0 ? ` · ${stop.distance_to_next_km} km` : ''}
                        </div>
                      )}
                    </div>
                  )}
...
const styles = {
  ...
  dayTab: {
    ...
    borderRadius: 99, background: 'transparent', color: 'var(--text-muted)',
  ...
  content:  { display: 'flex', height: 340 },
  stopList: {
  ...
  connector: {
    ...
    padding: '2px 6px 2px 17px', margin: '2px 0',
  },
