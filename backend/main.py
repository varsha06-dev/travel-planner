...
def get_db():
    ...
    host, port = (host_port.rsplit(":", 1) if ":" in host_port else (host_port, "6543"))
    ctx = ssl.create_default_context()
    ctx.check_hostname = False
    ctx.verify_mode = ssl.CERT_NONE
    return pg8000.dbapi.connect(
        host=host, port=int(port), database=dbname,
        user=user, password=password, ssl_context=ctx, timeout=10,
    )
    ...
    try:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                trip_info TEXT DEFAULT '{}', messages TEXT DEFAULT '[]'
            )
        """)
    finally:
        cur.close()
...
def deserialize_messages(raw):
    data = json.loads(raw); messages = []; visual_map = {}
...
def save_session(sid, messages, trip_info, visual_map=None):
    ...
    try:
        cur.execute("""
            ...
            ON CONFLICT (id) DO UPDATE
              SET messages=EXCLUDED.messages, trip_info=EXCLUDED.trip_info, updated_at=EXCLUDED.updated_at
...
def fetch_one_image(query):
    ...
    try:
        with urllib.request.urlopen(f"https://api.unsplash.com/search/photos?{params}", timeout=8) as r:
...
def fetch_place_images(places):
    if not places: return []
    def fetch(place): return {**place, "image": fetch_one_image(place["name"])}
    results = []
    with ThreadPoolExecutor(max_workers=5) as ex:
        for f in as_completed({ex.submit(fetch, p): p for p in places}):
            r = f.result()
...
def bbox_from_center(lat, lng, radius_km=80):
    dlat = radius_km/111.0; dlng = radius_km/(111.0*math.cos(math.radians(lat)))
    return [lng-dlng, lat-dlat, lng+dlng, lat+dlat]

def get_destination_center(destination):
    if not MAPBOX_TOKEN: return None
    url = (f"https://api.mapbox.com/geocoding/v5/mapbox.places/{urllib.parse.quote(destination)}.json"
           f"?access_token={MAPBOX_TOKEN}&limit=1&types=place,region,locality")
    try:
        with urllib.request.urlopen(url, timeout=6) as r: data = json.loads(r.read().decode())
        feats = data.get("features", [])
        if not feats: return None
        feat = feats[0]; lng, lat = feat["center"]
        bbox = feat.get("bbox") or bbox_from_center(lat, lng)
        return {"lng": round(lng,6), "lat": round(lat,6), "bbox": bbox}
    except: return None

def geocode(place_name, context="", proximity=None, bbox=None):
    if not MAPBOX_TOKEN: return None
    query = urllib.parse.quote(f"{place_name} {context}".strip())
    params = f"access_token={MAPBOX_TOKEN}&limit=1&types=poi,address,place"
    if proximity: params += f"&proximity={proximity['lng']},{proximity['lat']}"
    if bbox: params += f"&bbox={','.join(str(round(b,4)) for b in bbox)}"
    try:
        with urllib.request.urlopen(f"https://api.mapbox.com/geocoding/v5/mapbox.places/{query}.json?{params}", timeout=6) as r:
            data = json.loads(r.read().decode())
        feats = data.get("features", [])
        if not feats: return None
...
def get_directions(coords, mode="driving"):
...
def build_day_map(day_data, destination_context):
    ...
    def geocode_stop(stop):
        coords = geocode(stop["name"], destination_context, proximity=dest_info, bbox=dest_bbox)
        return {**stop, **(coords or {})}

    geocoded = []
    with ThreadPoolExecutor(max_workers=6) as ex:
        for f in as_completed({ex.submit(geocode_stop, s): s for s in stops}):
            geocoded.append(f.result())

    order = {s["name"]: i for i, s in enumerate(stops)}
    geocoded.sort(key=lambda s: order.get(s["name"], 99))
    valid = [s for s in geocoded if "lng" in s and "lat" in s and
             (not dest_info or haversine_km(dest_info["lat"], dest_info["lng"], s["lat"], s["lng"]) <= 120)]
...
def get_best_time_to_visit(destination: str) -> str:
    ...
    return str(TavilySearchResults(max_results=3).invoke(f"best time to visit {destination} weather seasons travel"))

@tool
def find_activities(destination: str, interests: Optional[str] = None) -> str:
    """Find top activities and attractions at a destination."""
...
def estimate_flight_costs(origin: str, destination: str, travel_dates: str) -> str:
    """Get flight cost estimates between two cities."""
    return str(TavilySearchResults(max_results=3).invoke(f"flight prices {origin} to {destination} {travel_dates} cost estimate"))

@tool
def estimate_hotel_costs(destination: str, budget_level: str = "mid-range") -> str:
    """Get hotel cost estimates for a destination."""
    return str(TavilySearchResults(max_results=3).invoke(f"{budget_level} hotels in {destination} price per night"))
...
Your process:
1. COLLECT INFO — Gather budget, travelers, duration, destination, interests, departure city, dates.
2. RESEARCH — Use tools to look up best times, activities, flight/hotel costs.
3. GENERATE PLAN — Create a detailed day-by-day itinerary with specific named places and costs.
4. REFINE — Accept feedback and update the plan.
Don't ask all questions at once. Always use real named places, hotels, restaurants, and attractions.
Format plans using markdown: ## headers, bullet points, **bold** for place names."""

EXTRACTION_PROMPT = """Extract the CURRENT trip details. Return ONLY JSON (null for anything not mentioned):
{{"destination":null,"travelers":null,"duration":null,"budget":null,"when":null,"departure_city":null}}
Conversation: {conversation}"""

PLACES_PROMPT = """Extract specific named places from this response (max 6).
Return ONLY a JSON array: [{{"name":"...","description":"under 10 words"}}]
Return [] if fewer than 2 named places. Response: {reply}"""

ITINERARY_PROMPT = """If this response contains a day-by-day itinerary with named locations, extract stops per day.
Return ONLY JSON (max 3 days, 5 stops each):
...
async def get_session(sid: str):
    ...
    for m in data:
        ...
        msgs.append(entry)
    return LoadResponse(session_id=sid, trip_info=json.loads(row["trip_info"] or "{}"), messages=msgs)
